...
<TouchableOpacity  ><Text  style={[styles.badge,{backgroundColor:'#e3eeeb',color:'256994',width:70,}]}>{item.date}</Text></TouchableOpacity>
<TouchableOpacity ><Text style={[styles.badge,{color: item.status === 'pending' ? 'black' : 'white', backgroundColor: item.status === 'completed' ? '256994' : item.status === 'pending' ? '#EAD72B' : item.status === 'Not Attended' ? '#EA2B2B' : 'transparent' }]}>{item.status}</Text></TouchableOpacity>
     </View>
